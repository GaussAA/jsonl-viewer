...
### 后续优化（已按建议落地，2026-09-20 v1.5.3）

- **P2-3 worker / 内存多开约束 → ✅ 已缓解**
  - **按 uri 引用计数复用 DataService**（`acquireService` / `releaseService` + `serviceRegistry`）：
    同一文件同时被「默认编辑器」与「命令面板」打开时不再重复建索引 / 重复起 worker / 重复占句柄；
    且 webview 内容重建后再挂载也不会重扫文件（引用计数归零才真正 dispose）。这是本项的**治本点**。
  - **worker 并发上限 `MAX_ACTIVE_WORKERS = 8`**：超出时新宿主退化主线程并告警。宁可极端场景牺牲部分流畅度，
    也不耗尽线程 / 内存。`WorkerIndexHost` 构造与释放维护全局计数（`released` 标志防重复递减）。
  - **`retainContextWhenHidden: true` 予以保留**，这是**有意取舍**：关掉可省 webview 常驻内存，
    但切标签会导致 webview 重建、切回有重载闪烁。有了上面的 DataService 复用，即便重建也不重扫文件；
    若后续更看重内存，可单独评估切换。
- **P2-8 非 UTF-8 编码提示 → ✅ 以低成本方式落地**
  不做不可靠的编码探测，改为：`getSampleFields` 返回「抽样 ≥20 行且有效记录为 0」时，
  横幅明确提示「文件可能不是 UTF-8 编码，或不是『每行一条 JSON』的 JSONL 格式」，
  把「满屏坏行」的困惑转成可执行解释。

**v1.5.3 验证**：`tsc --noEmit` 零错误 ｜ 全量 **150/150** ｜ 探针 **0 失败** ｜ 300MB 门禁全绿。
