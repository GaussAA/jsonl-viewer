...
- **虚拟滚动列表**：只渲染可视区 + overscan 的卡片（对象池回收），DOM 节点数 O(可视区)。
...
3. **虚拟滚动（`VirtualListLayout`）**：可变行高、实测覆盖；scrollTop↔行号双向定位为 O(可见区)；节流 + 合并调度，滚动再快也只发 1~2 个读批请求。
...
| 产物 | 体积 |
| --- | --- |
| `dist/extension.js`（扩展主进程） | ~17 KB |
| `dist/webview.js`（webview 前端） | ~47 KB |
